...
public:
    ...
    void dfs(int node, int zero, vector<bool>&vis, vector<int>&mark, unordered_map<int, vector<int>>&adj){
        vis[node] = true;
        mark[node] = zero;

        for(auto it : adj[node]){
            if(!vis[it]){
                dfs(it, !zero, vis, mark, adj);
            }
        }
    }
    ...
    vector<int> maxTargetNodes(vector<vector<int>>& edges1, vector<vector<int>>& edges2) {
        ...
        unordered_map<int, vector<int>> adj1 = findAdjList(edges1);
        unordered_map<int, vector<int>> adj2 = findAdjList(edges2);
